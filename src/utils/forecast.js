...
const forecast=(latitude,longitude,callback)=>{
      ...
      request({url , json:true} , (error,{body})=>{
        if(error){
          ...
          callback(undefined, body.current.weather[0].main + ', ' + body.current.weather[0].description + '. It is currently ' + body.current.temp + ' degrees out and the humidity is about ' + body.current.humidity + '%. ' + 'There is a ' + body.current.clouds + "% chance of rain.")
        }
      })
    }
